...
\abstract{How do people learn word meanings? One proposal, which we call \emph{contextual co-occurrence learning}, is that people constrain a word's meaning by taking into account its patterns of co-occurrence with other words. For example, one might conclude that \emph{postman} and \emph{mailman} are related words because they occur with similar sets of other words. Computational models suggest that, in principle, contextual co-occurrence learning is possible. However, a number of artificial language learning investigations (focused mainly on acquisition of syntactic category, rather than meaning) on a language called MNPQ have suggested that human capacity for contextual co-occurrence learning is quite limited, unless there are correlated cues in the input. Experiments in this paradigm typically present learners with words that are all unknown to start. By contrast, real language learners typically encounter input that contains some known words that adhere to a semantic organization, a property we call \emph{semantic coherence}. In three experiments, we show that semantic coherence facilitates contextual co-occurrence learning and that this effect crucially depends both on the presence of known words and the adherence of these known words to some semantic organization.}
