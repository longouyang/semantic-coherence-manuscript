...
In our analyses, memory was a significant (partial) mediator of the effect of semantic coherence on similarity and referent assignment scores, indicating that semantic coherence may reduce memory demands. This is consistent with previous work showing that artificial languages impose high memory burdens on learners and that facilitating memory improves learning \citep{frank2011}. 

interesting that memory for semantic baseline language is no different from 

We suspect that benefits of semantic coherence are fairly general, not limited to distributional learning. Indeed, shows that memory for new information is enhanced by consistency with prior knowledge, including results on phonology \citep{treiman1988}, narratives \tcite{bartlett32}, declarative facts \tcite{anderson1981}, complex skills like chess \tcite{chase1973}, and \textbf{something} \tcite{dewitt2012}.


 Distributional learning is not the only way in which language itself can provide evidence about meaning. Syntactic knowledge can strongly constraint meaning. For example, we might hear ``the rabbit glorped the duck'' and use morphology to infer that ``glorp'' is a verb. Because verbs typically denote actions, we can infer that ``glorp'' doesn't refer to a class of objects or substances in the world, eliminating a large set of possible meanings. Nevertheless, semantic coherence

 direct definitions.

 Warren, george in CHILDES: ``what's a plover''? ``it's a kind of bird''

 (contrast: ``what's a monoid''? ``it's a kind of semigroup'')

\textbf{might imagine screwing people (worse than chance) up by de-correlating coherence and co-occurrence}
...
\subsection{Distributional learning}

\paragraph{Mechanism}
Why does semantic coherence facilitate distributional learning---what is the mechanism? We speculate that learners might use semantic coherence to infer the topic of discourse and then attach meaning to novel words on the basis of co-occurrences with these topics. For example, in the 3/3 condition of our experiments, participants may have learned that the topic of discourse is either nature or travel and then tracked co-occurrence between these topics and novel words. If such a mechanism were at work, it might also predict that word learning would have a ``contiguous'' character, with faster learning for words that occur in more coherent contexts. \textbf{talk about hills network paper(s)}.

might be particularly helpful when you know nearly all of the words in a sentence/document.

\paragraph{Joint syntax, semantics}
\textbf{look into gillette, gleitman, gleitman, \& lederer, 1998}
\textbf{discuss syntax/semantics joint}

\paragraph{In the wild}
What do our experiments tell us about real language learners? First, we believe that distributional learning may not play a crucial role in the earliest stages of learning (e.g., the naming explosion), where visual referent information and social cues are available and predictive of meaning (\textbf{andrews, vigliocco, vinson}). However, distributional learning may be useful later on, for abstract words are defined in terms of relations (e.g., \emph{employee}), or concrete words whose referents are not available (e.g., \emph{taro}).

\textbf{try and connect back to blind studies}
