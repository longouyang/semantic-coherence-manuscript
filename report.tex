% generating author copy (floatsintext):
\documentclass[man,longtable,floatsintext]{my-apa6}
\usepackage[flushleft]{threeparttablex}
\let\ThreePartTableHere\ThreePartTable

% % generating journal manuscript (floatmark):
% \documentclass[man,longtable, floatmark]{my-apa6}
% \usepackage[flushleft]{threeparttablex}
% \let\ThreePartTableHere\ThreePartTable % make a copy of ThreePartTable that doesn't get floatmarked (for Table B1)
% \DeclareDelayedFloatFlavour{ThreePartTable}{table} % HT http://tex.stackexchange.com/a/114723/41154
...
% so doi shows up in typewriter
\renewenvironment{APACrefDOI}{\small \tt doi:}

% in captions, change Figure to Fig.
% HT http://tex.stackexchange.com/a/17490/41154
...
In contrast to these computational results, artificial language learning experiments suggest that distributional statistics \emph{alone} do not facilitate learning of linguistic categories.
  ...
  \begin{center}
    \begin{threeparttable}
      \begin{tabular}{ P{0.25\linewidth} P{0.375\linewidth} P{0.30\linewidth} }
        Exposure sentences\tnote{a} & Memory items & Similarity items\\
        \hline
      ...
      \end{tabular}
      \begin{tablenotes}
      \item [a] \footnotesize{Underlined sentences were withheld from exposure.}
      \end{tablenotes}
    \end{threeparttable}
    \label{mnpq-table}
  \end{center}
...
\begin{ThreePartTable}
  % see use of insertTableNotes at the end of longtable below
  % HT threeparttablex documentation
  \begin{TableNotes}
    \item \footnotesize{\emph{Note}. For readability, we divided the exposure values by 1,000. * indicates significant predictors ($p <$ 0.05).}
  \end{TableNotes}
...
\begin{table}[t]
  ...
  \begin{threeparttable}
    ...
    \end{tabular}
    \begin{tablenotes}
    \item \footnotesize{\emph{Note}. For interpretability of the intercept, we subtracted the lowest level of exposure from all exposure values. For readability, we divided exposure levels by 1,000. * indicates significant predictors ($p <$ 0.05).}
    \end{tablenotes}
  \end{threeparttable}
\end{table}

\subsubsection{Descriptive statistics for the memory task}
Fig.~\ref{x1-descriptive-mem} shows mean familiarity ratings broken down by sentence type, coherence, and exposures. As in Experiment 1a, we used Holm-corrected pairwise t-tests to compute a familiarity ordering on sentence types (Appendix~\ref{apx-t-tests}). The ordering for the nonce connective 3/3 condition was:
\begin{center}
  (F $\approx$ W) > C > P
\end{center}
...
We discarded the 51 participants who did not pass all the catch trials. Results are graphed in Fig. \ref{expt23-results}.
...
Fig.~\ref{x23-descriptive-mem} shows mean familiarity ratings broken down by sentence type, coherence, and exposures. As in previous analyses, we computed a familiarity ordering on sentence types within each phonological coherence condition using paired sample t-tests (Appendix~\ref{apx-t-tests}):

\begin{tabular}{l l}
Onset: & F $\approx$ W \, \, \, F > C, P \, \, \, (W $\approx$ C) > P\\
Rime:  & F $\approx$ W \, \, \, F > C, P \, \, \, (W $\approx$ C) > P\\
...
\noindent Participants in these conditions all failed to distinguish between withheld and co-occurrence violation sentences; they exhibited some learning of the language but not of the co-occurrence regularities.
...
\begin{table}[t]
  ...
  \begin{threeparttable}
    ...
    \end{tabular}
    \begin{tablenotes}
    \item \footnotesize{\emph{Note}. Frequencies were retrieved from Google N-grams data for the year 1999.}
    \end{tablenotes}
  \end{threeparttable}
\end{table}
...
F > W $\approx$ C > P
\end{center}
\noindent (see Table~\ref{familiarity-ordering-t-tests}). This indicates that participants were able to learn some regularities in the language, but not the co-occurrence regularities.

\subsubsection{Effect of exposure}

Results are graphed in Fig. \ref{expt23-results}. See Table \ref{expt23-regression} for regression results.
