...
Figure~\ref{x1-descriptive-mem} shows mean familiarity ratings broken down by sentence type, coherence, and exposures. Within each coherence level, we looked for evidence of a difficulty ordering on sentence types. For each participant, we computed their mean familiarity rating for each sentence type. Then, at each coherence level, we ran 6 paired-sample t-tests comparing mean familiarity ratings across sentence types (table TODO).
...
difficulty ordering in 0/3 was hard to establish, e.g., $f > w$ but $w \approx c$ and $f \approx c$.
