...
In our analyses, memory was a significant (albeit partial) mediator of the effect of semantic coherence on similarity and referent assignment scores. Thus, artificial languages may place too high a memory burden on learners \citep{frank2011}. Our finding is broadly consistent with work showing that memory for new information is enhanced by consistency with prior knowledge \tcite{bartlett32, anderson1981, zwaan1998, chase1973, dewitt2012}.

% \textbf{todo: better relate to categories of functions}
...
Using mediation analysis, we found evidence that reduced memory load appears to drive some, but not all, of the effect of semantic coherence. What other factors are in play? We speculate that learners might use semantic coherence to infer the topic of discourse and then attach meaning to novel words on the basis of co-occurrences with these topics. For example, in the 3/3 condition of our experiments, participants may have learned that the topic of discourse is either animals or vehicles and then tracked co-occurrence between these topics and novel words.

Through this kind of process, people might acquire meanings for abstract words (e.g., \emph{brigade}) or inchoate meanings for concrete words (e.g., people may know that \emph{taro} refers to some kind of plant). If such a mechanism were at work, it might also predict that word learning would have a ``contiguous'' character, with faster learning for words that occur in more coherent contexts. 
