...
\newcommand{\cas}[1]{ \textsf{\color{red} \small #1} }
...
\affiliation{Department of Psychology, Stanford University\vskip 5em Word count: 6,689}
...
\item car and wug
\item car and pif
\end{enumerate} might allow learners to infer that daxes and zivs are similar, as both words co-occur with ``dog'', and that wugs and pifs are similar, as both words co-occur with ``car.'' We emphasize that this small coherence manipulation does not elevate our stimuli to natural language, but it does render our stimuli more naturalistic than stimuli used in previous experiments. Artificial languages suitable for use in experiments simply cannot parallel the richness of natural language. Because of this, our work represents a first step towards measuring whether learners exposed to natural language form categories on the basis of co-occurrence structures.
...
\begin{figure}[t]
  \begin{center}
    ...
    \caption{Mean familiarity ratings for different sentence types in Experiment 1a and 1b.}
...
\begin{table}[ht]
  \caption{Regression model for Experiment 1a. For readability, exposure values were divided by 1000.}
...
Arguments to ``and'' often belong to the same syntactic and mostly semantic category.
...
In Experiment 1b, we investigated whether the familiar ``and'' connective drove the effects of coherence that we found in Experiment 1a.
We modified the materials from the 3/3 coherence condition, replacing ``and'' with a nonce connective, ``tezz''.
...
The materials were identical to the 3/3 condition of Experiment 1a, except we replaced the familiar ``and'' connective with a nonce connective ``tezz''.

\subsection{Design and Procedure}
The design and procedure were identical to that of Experiment 1a.

\subsection{Results and Discussion}
We excluded 9 subjects who did not pass all trials.

\begin{table}[t]
  \caption{Regression models from Experiment 1b. For readability, we divided exposure levels by 1000.}
  \label{fig:expt1b-regression}
  \centering
  \begin{tabular}{c c}
    \textbf{Memory} & \textbf{Similarity}\\
    {\small
    \begin{tabular}{l c c c r}
      \hline
      Predictor & $\beta$ & Std Error & $t$ & $p$ \\ \hline
      Intercept & 0.16 & 0.03 & 4.45 & <0.001*\\
      Exposures & 0.42 & 0.20 & 2.07 & <0.05* \\
      \hline
      \multicolumn{5}{l}{\T $R^2$ = 0.028, $F$(1, 149) = 4.31 \T}\\
    \end{tabular}
    } &
        {\small
        \begin{tabular}{l c c c r}
          \hline
          Predictor & $\beta$ & Std Error & $t$ & $p$ \\ \hline
          Intercept & 0.09 & 0.03 & 2.60 & <0.05*\\
          Exposures & 0.49 & 0.21 & 2.31 & <0.05* \\
          \hline
          \multicolumn{5}{l}{\T $R^2$ = 0.03, $F$(1, 149) = 5.34 \T}\\
        \end{tabular}
    }\\
    {\rule{0pt}{4ex}  \textbf{Referent assignment}} & \\
  {\small
    \begin{tabular}{l c c c r}
      \hline
      Predictor & $\beta$ & Std Error & $t$ & $p$ \\ \hline
      Intercept & 0.24 & 0.06 & 4.05 & <0.001*\\
      Exposures & 0.89 & 0.34 & 2.56 & <0.05* \\
      \hline
      \multicolumn{5}{l}{\T $R^2$ = 0.04, $F$(1, 147) = 0.04 \T}\\
    \end{tabular}  
  }\\
  \end{tabular}
\end{table}
